...
// Body + transient -> filter/drive
AudioConnection patchCord8(mixBody, 0, mixKick, 0);        // body -> mixKick ch0
AudioConnection patchCord9(mixKick, oscFilter);            // mixKick -> oscFilter
AudioConnection patchCord10(oscFilter, 0, oscDrive, 0);    // LP out -> oscDrive
AudioConnection patchCord11(oscDrive, 0, envAmp, 0);       // driven osc -> amp env
...
void loop() {
  ...
  float oscCutHzBase = 80.0f + oscCutoffNorm * 3920.0f;   // 80..4000 Hz
  ...
  // Osc distortion (oscDistNorm) on oscDrive
  ...
  // Smooth master volume
  float smoothRate = 0.08f;
