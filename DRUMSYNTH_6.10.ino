 ...
 * Version: 6.10
 * 
 * Teensy 4.x Drum Synthesizer - Kick/Snare with 909-style core and physical modeling snare
 * 
...
// SNARE: Physical modeling with modal synthesis
AudioSynthNoiseWhite     noiseSnare;           // dedicated noise for snare excitation
AudioFilterStateVariable snareModal1;          // modal filter 1 (fundamental ~180Hz)
AudioFilterStateVariable snareModal2;          // modal filter 2 (2nd mode ~360Hz)
AudioFilterStateVariable snareModal3;          // modal filter 3 (3rd mode ~540Hz)
AudioEffectEnvelope      envSnareModal1;
AudioEffectEnvelope      envSnareModal2;
AudioEffectEnvelope      envSnareModal3;

// SNARE: snare wires (high-frequency noise)
AudioFilterStateVariable snareWireFilter;
AudioEffectEnvelope      envSnareWire;
...
// Snare modal mixer
AudioMixer4              mixSnareModals; // mix modal resonators
...
// Snare: Physical modeling - noise excites modal filters
AudioConnection patchCord16(noiseSnare, snareModal1);
AudioConnection patchCord17(noiseSnare, snareModal2);
AudioConnection patchCord18(noiseSnare, snareModal3);
AudioConnection patchCord19(snareModal1, 2, envSnareModal1, 0);  // bandpass output
AudioConnection patchCord20(snareModal2, 2, envSnareModal2, 0);  // bandpass output
AudioConnection patchCord21(snareModal3, 2, envSnareModal3, 0);  // bandpass output
AudioConnection patchCord22(envSnareModal1, 0, mixSnareModals, 0);
AudioConnection patchCord23(envSnareModal2, 0, mixSnareModals, 1);
AudioConnection patchCord24(envSnareModal3, 0, mixSnareModals, 2);
AudioConnection patchCord25(mixSnareModals, 0, mixFinal, 2);  // modal body -> final mix

// Snare: snare wires
AudioConnection patchCord26(noiseSnare, snareWireFilter);
AudioConnection patchCord27(snareWireFilter, 2, envSnareWire, 0);  // high-pass output
AudioConnection patchCord28(envSnareWire, 0, mixFinal, 3);  // snare wires -> final mix

// Master drive -> outputs
AudioConnection patchCord29(mixFinal, 0, driveFX, 0);
AudioConnection patchCord30(driveFX, 0, i2s1, 0);
AudioConnection patchCord31(driveFX, 0, i2s1, 1);
...
// Snare physical modeling parameters
float snareFundamentalNorm = 0.36f;    // SF - fundamental frequency (0..1 -> 100..400 Hz)
float snareModal1LevelNorm = 1.0f;     // SM1 - modal 1 level (fundamental)
float snareModal2LevelNorm = 0.6f;     // SM2 - modal 2 level (2nd harmonic)
float snareModal3LevelNorm = 0.3f;     // SM3 - modal 3 level (3rd harmonic)
float snareDecayBaseNorm   = 0.15f;    // SD - base decay time (all modes scaled from this)
float snareAttackNorm      = 0.0f;     // ST - attack time

// Snare wires
float snareWireLevelNorm    = 0.7f;    // SWL - snare wire level
float snareWireCutoffNorm   = 0.4f;    // SWC - snare wire cutoff (high-pass, 2000..8000 Hz)
float snareWireResonanceNorm= 0.3f;    // SWR - snare wire resonance
float snareWireDecayNorm    = 0.12f;   // SWD - snare wire decay (usually faster than body)
...
void parseSerialLine(const String &line) {
  ...
  // Snare physical modeling - modal synthesis
  else if (c0 == 'S' && c1 == 'F') snareFundamentalNorm = val;   // SF - Snare Fundamental
  else if (c0 == 'S' && c1 == '1') snareModal1LevelNorm = val;   // S1 - Modal 1 level
  else if (c0 == 'S' && c1 == '2') snareModal2LevelNorm = val;   // S2 - Modal 2 level  
  else if (c0 == 'S' && c1 == '3') snareModal3LevelNorm = val;   // S3 - Modal 3 level
  else if (c0 == 'S' && c1 == 'D') snareDecayBaseNorm   = val;   // SD - Snare Decay base
  else if (c0 == 'S' && c1 == 'T') snareAttackNorm      = val;   // ST - Snare attack

  // Snare wires - use W prefix
  else if (c0 == 'W' && c1 == 'L') snareWireLevelNorm    = val;  // WL - Wire Level
  else if (c0 == 'W' && c1 == 'C') snareWireCutoffNorm   = val;  // WC - Wire Cutoff
  else if (c0 == 'W' && c1 == 'R') snareWireResonanceNorm= val;  // WR - Wire Resonance
  else if (c0 == 'W' && c1 == 'D') snareWireDecayNorm    = val;  // WD - Wire Decay
...
void triggerSnare() {
  envSnareModal1.noteOn();
  envSnareModal2.noteOn();
  envSnareModal3.noteOn();
  envSnareWire.noteOn();
}
...
void setup() {
  ...
  // Snare physical modeling setup
  noiseSnare.amplitude(1.0f);  // Full amplitude, will be controlled via envelopes
  
  // Modal filters: bandpass mode, high Q for resonance
  snareModal1.octaveControl(2.0f);  // bandpass mode
  snareModal2.octaveControl(2.0f);  // bandpass mode
  snareModal3.octaveControl(2.0f);  // bandpass mode
  
  // Snare wire filter: high-pass mode
  snareWireFilter.octaveControl(3.0f);  // high-pass mode
  
  // Initial envelope settings (will be updated in loop)
  envSnareModal1.attack(0.0f);
  envSnareModal1.decay(200.0f);
  envSnareModal1.sustain(0.0f);
  envSnareModal1.release(0.0f);
  
  envSnareModal2.attack(0.0f);
  envSnareModal2.decay(150.0f);  // Higher modes decay faster
  envSnareModal2.sustain(0.0f);
  envSnareModal2.release(0.0f);
  
  envSnareModal3.attack(0.0f);
  envSnareModal3.decay(100.0f);  // Highest mode decays fastest
  envSnareModal3.sustain(0.0f);
  envSnareModal3.release(0.0f);
  
  envSnareWire.attack(0.0f);
  envSnareWire.decay(160.0f);
  envSnareWire.sustain(0.0f);
  envSnareWire.release(0.0f);
  
  // Snare modal mixer initial gains
  mixSnareModals.gain(0, 1.0f);  // modal 1
  mixSnareModals.gain(1, 0.6f);  // modal 2
  mixSnareModals.gain(2, 0.3f);  // modal 3
  mixSnareModals.gain(3, 0.0f);
...
oscTri2.frequency(f3);
  ...
  // Snare physical modeling - modal synthesis
  // Fundamental frequency (base drum head mode)
  float snareFundamental = 100.0f + snareFundamentalNorm * 300.0f;  // 100..400 Hz
  
  // Modal frequencies: fundamental and harmonics
  float modal1Freq = snareFundamental;        // 1x (fundamental)
  float modal2Freq = snareFundamental * 2.0f; // 2x (second mode)
  float modal3Freq = snareFundamental * 3.0f; // 3x (third mode)
  
  // Set modal filter frequencies and Q (high Q for resonance)
  float modalQ = 10.0f;  // High Q for drum-like resonance
  snareModal1.frequency(modal1Freq);
  snareModal1.resonance(modalQ);
  
  snareModal2.frequency(modal2Freq);
  snareModal2.resonance(modalQ);
  
  snareModal3.frequency(modal3Freq);
  snareModal3.resonance(modalQ);
  
  // Base decay time, higher modes decay faster (realistic drum physics)
  float snareBaseDecayMs = 10.0f + snareDecayBaseNorm * 500.0f;  // 10..510 ms
  float modal1Decay = snareBaseDecayMs;                          // Fundamental decays slowest
  float modal2Decay = snareBaseDecayMs * 0.75f;                  // 2nd mode 25% faster
  float modal3Decay = snareBaseDecayMs * 0.5f;                   // 3rd mode 50% faster
  
  float snareAtkMs = snareAttackNorm * 20.0f;  // 0..20 ms (very fast attack for snare)
  
  envSnareModal1.attack(snareAtkMs);
  envSnareModal1.decay(modal1Decay);
  envSnareModal1.sustain(0.0f);
  envSnareModal1.release(0.0f);
  
  envSnareModal2.attack(snareAtkMs);
  envSnareModal2.decay(modal2Decay);
  envSnareModal2.sustain(0.0f);
  envSnareModal2.release(0.0f);
  
  envSnareModal3.attack(snareAtkMs);
  envSnareModal3.decay(modal3Decay);
  envSnareModal3.sustain(0.0f);
  envSnareModal3.release(0.0f);
  
  // Update modal mixer gains (level controls)
  mixSnareModals.gain(0, snareModal1LevelNorm);
  mixSnareModals.gain(1, snareModal2LevelNorm);
  mixSnareModals.gain(2, snareModal3LevelNorm);
  
  // Snare wires - high-pass filtered noise
  float snareWireFreq = 2000.0f + snareWireCutoffNorm * 6000.0f;  // 2000..8000 Hz
  snareWireFilter.frequency(snareWireFreq);
  
  float snareWireQ = 0.7f + snareWireResonanceNorm * 3.0f;        // 0.7..3.7
  snareWireFilter.resonance(snareWireQ);
  
  float snareWireDecayMs = 10.0f + snareWireDecayNorm * 200.0f;   // 10..210 ms (wires decay quickly)
  envSnareWire.attack(0.0f);
  envSnareWire.decay(snareWireDecayMs);
  envSnareWire.sustain(0.0f);
  envSnareWire.release(0.0f);
  ...
  mixFinal.gain(0, mixFinalBaseGains[0] * masterGainSmooth);  // kick body
  mixFinal.gain(1, mixFinalBaseGains[1] * masterGainSmooth);  // kick noise
  mixFinal.gain(2, mixFinalBaseGains[2] * masterGainSmooth);  // snare modals (body)
  mixFinal.gain(3, mixFinalBaseGains[3] * masterGainSmooth * snareWireLevelNorm);  // snare wires (with level control)
}
