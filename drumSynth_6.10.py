...
class DrumGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Teensy Kick/Snare Drum Synthesizer")
        self.style = None
        ...
        # KICK SECTION: Columns 1-2
        kick_col1_frame = ttk.Frame(main_frame)
        kick_col1_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        kick_col1_frame.columnconfigure(1, weight=1)

        kick_col2_frame = ttk.Frame(main_frame)
        kick_col2_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 5))
        kick_col2_frame.columnconfigure(1, weight=1)

        # SNARE SECTION: Columns 3-4
        snare_col1_frame = ttk.Frame(main_frame)
        snare_col1_frame.grid(row=0, column=2, sticky="nsew", padx=(5, 5))
        snare_col1_frame.columnconfigure(1, weight=1)

        snare_col2_frame = ttk.Frame(main_frame)
        snare_col2_frame.grid(row=0, column=3, sticky="nsew", padx=(5, 0))
        snare_col2_frame.columnconfigure(1, weight=1)

        # ---------- SINE A PITCH (OSC 0) ----------
        sineA_frame = ttk.LabelFrame(kick_col1_frame, text="Sine A (Osc 0) Pitch Env")
        ...
        # ---------- SINE B PITCH (OSC 1) ----------
        sineB_frame = ttk.LabelFrame(kick_col1_frame, text="Sine B (Osc 1) Pitch Env")
        ...
        # ---------- OSC MIX FRAME ----------
        mix_frame = ttk.LabelFrame(kick_col1_frame, text="Oscillator Mix")
        ...
        # ---------- AMP A FRAME ----------
        amp_frame = ttk.LabelFrame(kick_col1_frame, text="AMP A (Kick Amp Env)")
        ...
        # ---------- TRANSIENT (TICK/CLAP) ----------
        transient_frame = ttk.LabelFrame(kick_col1_frame, text="Transient")
        ...
        # ---------- TRI A PITCH (OSC 2) ----------
        triA_frame = ttk.LabelFrame(kick_col2_frame, text="Tri A (Osc 2) Pitch Env")
        ...
        triB_frame = ttk.LabelFrame(kick_col2_frame, text="909 Core (Osc 3) Pitch Env")
        ...
        # ---------- NOISE ENGINE ----------
        noise_frame = ttk.LabelFrame(kick_col2_frame, text="Noise Engine")
        ...
        # ---------- DISTORTION ----------
        dist_frame = ttk.LabelFrame(kick_col2_frame, text="Distortion")
        ...
        oscTone_frame = ttk.LabelFrame(kick_col2_frame, text="OSC Tone (Post-Osc Mix)")
        ...
        filtEnv_frame = ttk.LabelFrame(kick_col2_frame, text="Filter Envelope")
        ...
        # ---------- RANDOMIZATION ----------
        rand_frame = ttk.LabelFrame(kick_col2_frame, text="Randomization")
        ...
        # ---------- SNARE SECTION (Physical Modeling) ----------
        
        # Snare Modal (Drum Head Modes)
        snareModal_frame = ttk.LabelFrame(snare_col1_frame, text="Snare Modal (Drum Head)")
        snareModal_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=5)
        snareModal_frame.columnconfigure(1, weight=1)
        
        self.sf = tk.DoubleVar(value=0.36)    # SF - Fundamental frequency
        self.s1 = tk.DoubleVar(value=1.0)     # S1 - Modal 1 level
        self.s2 = tk.DoubleVar(value=0.6)     # S2 - Modal 2 level
        self.s3 = tk.DoubleVar(value=0.3)     # S3 - Modal 3 level
        self.sd = tk.DoubleVar(value=0.15)    # SD - Decay base
        self.st = tk.DoubleVar(value=0.0)     # ST - Attack
        
        ttk.Label(snareModal_frame, text="Fundamental").grid(row=0, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.sf,
                 command=self.on_sf).grid(row=0, column=1, sticky="ew")
        
        ttk.Label(snareModal_frame, text="Modal 1 Level").grid(row=1, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.s1,
                 command=self.on_s1).grid(row=1, column=1, sticky="ew")
        
        ttk.Label(snareModal_frame, text="Modal 2 Level").grid(row=2, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.s2,
                 command=self.on_s2).grid(row=2, column=1, sticky="ew")
        
        ttk.Label(snareModal_frame, text="Modal 3 Level").grid(row=3, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.s3,
                 command=self.on_s3).grid(row=3, column=1, sticky="ew")
        
        ttk.Label(snareModal_frame, text="Decay Base").grid(row=4, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.sd,
                 command=self.on_sd).grid(row=4, column=1, sticky="ew")
        
        ttk.Label(snareModal_frame, text="Attack").grid(row=5, column=0, sticky="w")
        tk.Scale(snareModal_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.st,
                 command=self.on_st).grid(row=5, column=1, sticky="ew")
        
        # Snare Wires
        snareWire_frame = ttk.LabelFrame(snare_col1_frame, text="Snare Wires")
        snareWire_frame.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)
        snareWire_frame.columnconfigure(1, weight=1)
        
        self.wl = tk.DoubleVar(value=0.7)     # WL - Wire Level
        self.wc = tk.DoubleVar(value=0.4)     # WC - Wire Cutoff
        self.wr = tk.DoubleVar(value=0.3)     # WR - Wire Resonance
        self.wd = tk.DoubleVar(value=0.12)    # WD - Wire Decay
        
        ttk.Label(snareWire_frame, text="Level").grid(row=0, column=0, sticky="w")
        tk.Scale(snareWire_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.wl,
                 command=self.on_wl).grid(row=0, column=1, sticky="ew")
        
        ttk.Label(snareWire_frame, text="Cutoff").grid(row=1, column=0, sticky="w")
        tk.Scale(snareWire_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.wc,
                 command=self.on_wc).grid(row=1, column=1, sticky="ew")
        
        ttk.Label(snareWire_frame, text="Resonance").grid(row=2, column=0, sticky="w")
        tk.Scale(snareWire_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.wr,
                 command=self.on_wr).grid(row=2, column=1, sticky="ew")
        
        ttk.Label(snareWire_frame, text="Decay").grid(row=3, column=0, sticky="w")
        tk.Scale(snareWire_frame, from_=0, to=1, resolution=0.01,
                 orient="horizontal", variable=self.wd,
                 command=self.on_wd).grid(row=3, column=1, sticky="ew")

        # ---------- MAIN VOLUME (bottom right) ----------
        volume_frame = ttk.LabelFrame(snare_col2_frame, text="Output")
        volume_frame.grid(row=0, column=0, columnspan=2, sticky="ews", pady=(5, 0))
        ...
        # Make the snare_col2_frame expand to push volume to bottom
        snare_col2_frame.rowconfigure(0, weight=1)
    ...
    def send_all_params(self):
        ...
        # Snare Modal (Physical Modeling)
        self.send_cmd("SF", self.sf.get())
        self.send_cmd("S1", self.s1.get())
        self.send_cmd("S2", self.s2.get())
        self.send_cmd("S3", self.s3.get())
        self.send_cmd("SD", self.sd.get())
        self.send_cmd("ST", self.st.get())
        
        # Snare Wires
        self.send_cmd("WL", self.wl.get())
        self.send_cmd("WC", self.wc.get())
        self.send_cmd("WR", self.wr.get())
        self.send_cmd("WD", self.wd.get())
    ...
    # Snare Modal callbacks
    def on_sf(self, _): self.send_cmd("SF", self.sf.get())  # SF - Snare Fundamental
    def on_s1(self, _): self.send_cmd("S1", self.s1.get())  # S1 - Modal 1 level
    def on_s2(self, _): self.send_cmd("S2", self.s2.get())  # S2 - Modal 2 level
    def on_s3(self, _): self.send_cmd("S3", self.s3.get())  # S3 - Modal 3 level
    def on_sd(self, _): self.send_cmd("SD", self.sd.get())  # SD - Decay base
    def on_st(self, _): self.send_cmd("ST", self.st.get())  # ST - Attack
    
    # Snare Wire callbacks
    def on_wl(self, _): self.send_cmd("WL", self.wl.get())  # WL - Wire Level
    def on_wc(self, _): self.send_cmd("WC", self.wc.get())  # WC - Wire Cutoff
    def on_wr(self, _): self.send_cmd("WR", self.wr.get())  # WR - Wire Resonance
    def on_wd(self, _): self.send_cmd("WD", self.wd.get())  # WD - Wire Decay
